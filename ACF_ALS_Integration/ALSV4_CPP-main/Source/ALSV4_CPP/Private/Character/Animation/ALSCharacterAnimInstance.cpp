...
void UALSCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if (MovementState.Grounded())
	{
		...
		if (prevShouldMove == false && Grounded.bShouldMove)
		{
			...
			CurrentMoveset.ALSOverlay.TurnInPlace.ElapsedDelayTime = 0.0f;
...
void UALSCharacterAnimInstance::TurnInPlaceCheck(float DeltaSeconds)
{
	...
	if (FMath::Abs(AimingValues.AimingAngle.X) <= CurrentMoveset.ALSOverlay.TurnInPlace.TurnCheckMinAngle ||
		CharacterInformation.AimYawRate >= CurrentMoveset.ALSOverlay.TurnInPlace.AimYawRateLimit)
	{
		CurrentMoveset.ALSOverlay.TurnInPlace.ElapsedDelayTime = 0.0f;
		return;
	}

	CurrentMoveset.ALSOverlay.TurnInPlace.ElapsedDelayTime += DeltaSeconds;
	const float ClampedAimAngle = FMath::GetMappedRangeValueClamped({ CurrentMoveset.ALSOverlay.TurnInPlace.TurnCheckMinAngle, 180.0f},
	                                                                {
																		CurrentMoveset.ALSOverlay.TurnInPlace.MinAngleDelay,
																		CurrentMoveset.ALSOverlay.TurnInPlace.MaxAngleDelay
	                                                                },
	...
	if (CurrentMoveset.ALSOverlay.TurnInPlace.ElapsedDelayTime > ClampedAimAngle)
	{
                                            ...
                                            bool OverrideCurrent)
{
	...
	FACFTurnInPlaceAsset TargetTurnAsset;

		// Step 2: Choose Turn Asset based on the Turn Angle and Stance;

	if (Stance.Standing())
	{
		if (FMath::Abs(TurnAngle) < CurrentMoveset.ALSOverlay.TurnInPlace.Turn180Threshold)
		{
		
				TargetTurnAsset = TurnAngle < 0.0f
					? CurrentMoveset.ALSOverlay.TurnInPlace.N_TurnIP_L90
					: CurrentMoveset.ALSOverlay.TurnInPlace.N_TurnIP_R90;
		}
		else
		{
			
				TargetTurnAsset = TurnAngle < 0.0f
					? CurrentMoveset.ALSOverlay.TurnInPlace.N_TurnIP_L180
					: CurrentMoveset.ALSOverlay.TurnInPlace.N_TurnIP_R180;
	...
	else
	{
		...
		{
			TargetTurnAsset = TurnAngle < 0.0f
				                  ? CurrentMoveset.ALSOverlay.TurnInPlace.CLF_TurnIP_L90
				                  : CurrentMoveset.ALSOverlay.TurnInPlace.CLF_TurnIP_R90;
		}
		else
		{
			TargetTurnAsset = TurnAngle < 0.0f
				                  ? CurrentMoveset.ALSOverlay.TurnInPlace.CLF_TurnIP_L180
				                  : CurrentMoveset.ALSOverlay.TurnInPlace.CLF_TurnIP_R180;
		}
	}
